...
- 2026-08-23 — `HOST-CONTAINER-FILL-01`: added a `container` Shell layout that fills the Host-assigned block. Host navigation and safe-area space are reserved exactly once by the Host rather than inferred from application-specific viewport subtraction inside the framework.
- 2026-08-23 — `MOBILE-INPUT-SCALE-01`: established a minimum computed `16px` font size for editable framework controls on mobile/coarse-pointer clients so focusing the Composer does not trigger readability zoom in iPhone Safari. Conforming Hosts MUST NOT solve this by disabling user zoom.
...
**Acceptance:** panel, drawer, inline, and full-page modes work at mobile and desktop widths; `shell_layout: viewport | container` lets a standalone surface use the framework viewport baseline or an embedded surface fill one definite Host-assigned block without application-specific navigation arithmetic; the Host reserves its navigation and safe-area space exactly once; the Host can set Shell title/subtitle, typed user-facing labels, and size through public props and theme variables, and can hide maintainer settings from ordinary users without changing their configured values; `ComposerToolbar` renders the fit-assistant default `Plus`, `Mic`, and `ArrowUp` actions as one button group placed below the text field or vertically beside it; the text field itself is the only text-mode control; `Plus` owns the attachment/camera menu; the microphone is hold-to-talk and uses capability-gated live dictation or voice-message behavior; editable controls compute to at least `16px` on mobile/coarse-pointer clients while browser zoom remains user-controllable; English defaults remain complete when no label overrides are supplied; the published Vue package exposes one documented default stylesheet import and contains library artifacts rather than Reference Host application assets; fallback component layout selectors are scoped to their component root and MUST NOT alter Host-provided content or Action renderers; Host-driven active-Conversation changes rebind the thread without adding framework-owned navigation; keyboard and screen-reader paths cover every operation; chronology, multimodal flows, both voice modes, Privacy Center, tool visibility, six disclosure levels, and `raw_trace` availability remain consistent.
...
- verify `container` Shells fill the Host-assigned block up to its navigation boundary without double-reserving safe-area space, and mobile/coarse-pointer editable controls compute to at least `16px` without a viewport rule that disables user zoom;
