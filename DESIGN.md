...
- Evidence reviewed: the public MVP specification, the Vue reference implementation, browser E2E and visual QA at 320/390 px and desktop widths, deployment screenshots from the household-finance Pilot showing the input-mode select, oversized non-previewable Draft Attachment, duplicated space above fixed Host tabs, and iPhone Safari focus zoom, domain fixtures for wellness/itinerary/household finance, and common accessibility/recovery requirements.
...
- Goals: give current and future applications one high-standard embedded AI assistant instead of repeatedly rebuilding chat, streaming, tools, Action execution policy, Memory, attachments, and recovery.
...
- Goals: provide default-disabled Host Data Write Tools that standard applications can enable through reviewed schema, scope, and execution policy instead of custom plugin code.
...
- Success signals: a new Host can embed the basic assistant in one working day; a standard CRUD-style application can add confirmed or reviewed automatic Actions through an approved Integration Manifest without custom plugin code; a custom plugin can still add specialized behavior without changing core; users can inventory/export/delete registered assistant data from one place; no model, generated integration, or plugin can bypass Host execution policy, authorization, or transaction boundaries.
...
- Principle 1: the framework owns assistant behavior consistency; the Host owns business authority and supplies one immutable actor/scope/page-context snapshot for each Run.
...
- Principle 4: model output cannot bypass Host execution policy; every business change is a typed Action and is either user-confirmed or automatically applied through a pre-approved, bounded, idempotent, transactional, visible, and audited Host policy.
...
- Principle 10: Host Data Write Tools are default-off, schema-bound, scope-bound, and Action-wrapped; the model never receives arbitrary SQL, database credentials, or unrestricted table access.
...
- New/changed components: headless assistant store, assistant shell, conversation thread, `MessageTimeDivider`, message-part renderers, multimodal composer, configurable `ComposerToolbar`, compact previewable `AttachmentTray`, `AttachmentGrid`, `AttachmentFileCard`, `AttachmentProcessingStatus`, `AttachmentLightbox`, `VoiceMessageBubble`, `LiveDictationControl`, `TranscriptionStatus`, reasoning disclosure, tool activity, citations, Action Workspace, `ExecutionModeSettings`, `HostDataToolSettings`, `AutoAppliedResultCard`, `PrivacyCenter`, `PrivacyJobStatus`, error recovery, generic content slots, `action_type`-keyed Action renderer slots, Host-context badges, Integration Manifest editor/review, generated-risk report, Context Profile settings, Context Manifest inspector, and developer inspectors.
...
voice_input:
  ...
  live_dictation_adapter: embedded_model | api | unavailable
```
...
- Page context is captured when Send begins, remains immutable for that Run, and may be shown as a Host-context badge. Switching month, record, or route affects the next Run only.
...
Custom Providers receive the authorized Run Host Context so they can request private media through the Host boundary. They never infer actor/scope from an Attachment ID. Apply, undo, and refresh remain Host Adapter callbacks; the default UI shows undo only when the Host declares it.

Reference plugins and renderers are registered by the reference Host, never by Core storage. A Host may replace Action presentation by `action_type` while reusing the same Headless Action state and confirm/edit/cancel/undo events.

The same rule applies to tools: a mounted Host begins with domain-neutral Essentials only. Reference record, reference knowledge, generated, and domain tools are explicit Host registrations, so the capability panel never advertises tools backed by another application's sample storage.
...
- `read_only` shows analysis and non-executable recommendations. Write attempts explain that the current mode prevents mutation.
- `confirm_each` is the default. Editable Action cards display impact, source, validation, and explicit confirm/cancel controls.
- `auto_apply_allowlist` may automatically apply only reviewed low-risk Action types inside configured actor/scope, impact, confidence, rate, idempotency, transaction, and compensation bounds.

`ExecutionModeSettings` is a Host or user-policy surface, never a model-controlled choice. It lists the current mode, allowlisted Action types, limits, forced-confirmation categories, automatic history, and undo availability.

`HostDataToolSettings` is a developer/maintainer surface and is disabled by default. It lists approved entities, create/update/upsert/delete/link/unlink operations, writable fields, validation schema, actor/tenant row scope, optimistic concurrency, execution mode, limits, and compensation. Enabling one entity or operation never enables others. Raw SQL, connection details, unrestricted filters, and undeclared fields are never exposed to the model.
...
- Layout adaptations: `viewport` Shells use the framework standalone height baseline; `container` Shells fill one definite Host-assigned block with `height: 100%` and `min-height: 0`; Host navigation, safe-area, and keyboard space are reserved exactly once by the Host and are never encoded as application-specific subtraction inside the framework; the composer pins only when the Host container permits it; the Composer Toolbar stays one horizontal row in `below` mode or one 44 px vertical rail in `side` mode; Attachment Tray uses a bounded horizontal strip or compact wrap without covering text/actions; grids recalculate columns without horizontal page overflow; Lightbox fills the safe viewport; tables and domain cards provide narrow-width fallbacks.
...
- Loading: distinguish history loading, sending, model streaming, tool execution, attachment validation/optimization/upload/processing, voice-message batch transcription, live-dictation model preparation/streaming transcription, Action policy evaluation/confirmed application/automatic application, and Privacy inventory/export/deletion jobs.
- Empty: show host-provided starter prompts and current-context hints, never fabricated conversation content.
- Error: preserve the text draft, ordered Draft Attachments, sent Message attachments, playable voice Message, partial dictation text, and safe metadata as applicable; distinguish validation, optimization, upload, processing, preview, permission, disabled/unauthorized Host Data Tool, optimistic-concurrency conflict, missing Host target, execution-policy denial/bounds, and Assistant Run failure; `run.failed` and standard Host HTTP error details become visible non-empty errors; Host apply/undo failures stay on the typed Action card instead of escaping as an unhandled request; Privacy Jobs retain item-level success/failure and unresolved-processor evidence; explain whether refresh/rebase, confirmation, retry, remove-and-continue, cancel, transcription correction, privacy retry, reconcile, edit, archival, compatible plugin re-enable, or context-profile fallback is available.
- Success: terminal Runs clear transient thinking-stage indicators; confirmed and automatically applied Action cards identify execution mode, show the user-facing outcome, and invoke an explicit Host refresh callback after apply and undo; Privacy Jobs summarize exported/deleted categories and remaining restrictions without claiming more than processors confirmed.
...
- Terminology: “助手”, “数据写入工具”, “当前未启用”, “允许创建”, “允许更新”, “版本冲突”, “刷新后重试”, “只读模式”, “逐次确认”, “自动应用”, “已自动完成”, “需要确认”, “策略阻止”, “撤销”, “附件”, “正在校验”, “正在压缩”, “正在上传”, “正在解析”, “解析失败”, “不支持预览”, “查看原图”, “保存为业务资料”, “来源附件”, “隐私中心”, “导出助手数据”, “删除助手数据”, “删除影响”, “部分完成”, “保留期限”, “语音消息”, “实时转写”, “正在录音”, “正在转写”, “转写失败”, “正在思考”, “正在查看”, “待确认”, “确认”, “修改”, “取消”, “已应用”, “恢复回复”, “引用来源”.
...
- 2026-08-23 — `HOST-CONTAINER-FILL-01`: embedded layouts use an explicit `container` Shell that fills a definite Host block. The Host owns navigation and safe-area reservation once; the framework does not learn application-specific Tab heights or viewport subtraction constants.
- 2026-08-23 — `MOBILE-INPUT-SCALE-01`: editable framework controls retain a minimum computed `16px` font size on mobile/coarse-pointer clients so iPhone Safari focus remains stable. Browser zoom stays enabled for accessibility.
